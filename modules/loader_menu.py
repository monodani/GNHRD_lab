#!/usr/bin/env python3
"""
경상남도인재개발원 RAG 챗봇 - 식단표 벡터스토어 로더 (간소화)

menu.csv → FAISS 벡터스토어 생성 (CSV 방식)
"""
...
# 경로 설정
SOURCE_FILE = "data/menu/menu.csv"
...
class MenuLoader:
    ...
    def __init__(self):
        ...
        logger.info("MenuLoader 간소화 버전 초기화")
    ...
    def build_vectorstore(self, force_rebuild: bool = False) -> bool:
        ...
        try:
            logger.info("CSV 로드 및 분할 시작...")
            
            # CSV 로드
            loader = CSVLoader(str(self.source_file))
            docs = loader.load()
            ...
            # 저장 (menu_index.faiss, menu_index.pkl)
