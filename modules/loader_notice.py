#!/usr/bin/env python3
"""
경상남도인재개발원 RAG 챗봇 - 공지사항 벡터스토어 로더 (간소화)

notice.txt → FAISS 벡터스토어 생성 (TextLoader 방식)
"""
...
class NoticeLoader:
    ...
    def __init__(self):
        ...
        logger.info("NoticeLoader 간소화 버전 초기화")
    
    def build_vectorstore(self, force_rebuild: bool = False) -> bool:
        """벡터스토어 빌드 (코랩 방식)"""
        if not TEXT_AVAILABLE:
            raise ValueError("필요한 라이브러리 설치: pip install langchain_community faiss-cpu langchain_openai")
        
        if not self.source_file.exists():
            raise ValueError(f"파일 없음: {self.source_file}")
        
        try:
            logger.info("텍스트 로드 및 분할 시작...")
            
            # 텍스트 로드
            loader = TextLoader(str(self.source_file))
            docs = loader.load()
            ...
            # 저장 (notice_index.faiss, notice_index.pkl)
