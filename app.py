...
# app.py 맨 아래 (main() 함수 다음)에 추가


---

def debug_faiss_structure():
    """FAISS 구조 디버깅"""
    print("🔍 FAISS 내부 구조 검증")
    ...
    try:
        ...
        print(f"✅ vectorstore 타입: {type(vectorstore)}")
        print(f"✅ index 속성: {hasattr(vectorstore, 'index')}")
        print(f"✅ docstore 속성: {hasattr(vectorstore, 'docstore')}")
        print(f"✅ embeddings 속성: {hasattr(vectorstore, 'embeddings')}")
        
        if hasattr(vectorstore, 'index'):
            print(f"✅ FAISS 벡터 개수: {vectorstore.index.ntotal}")
