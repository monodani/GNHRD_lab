...
"""
벼리톡@경상남도인재개발원 - 사이버교육 핸들러 v5.1 (직접 FAISS 접근)
...
# 추가 설정 (필요시 확장)
# FUTURE: 사이버교육 특화 로직이 필요하면 여기에 추가
...
class CyberHandler:
    """사이버교육 검색 핸들러 (직접 FAISS 접근)"""
    
    def __init__(self):
        self.threshold = HANDLER_THRESHOLDS["cyber"]
        self.department_info = DEPARTMENT_CONTACTS["cyber"]
        self.index_manager = get_index_manager()
        logger.info(f"CyberHandler v5.1 초기화 완료 (임계값: {self.threshold})")
    
    def search_chunks(self, query: str) -> List[ChunkResult]:
        """직접 FAISS 접근으로 사이버교육 검색"""
        try:
