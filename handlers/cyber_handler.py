...
"""
벼리톡@경상남도인재개발원 - 사이버교육 핸들러 v1.0 (리팩토링)
...
class CyberHandler(BasePandasAgentHandler):
    """
    사이버교육 CSV 데이터를 분석하는 핸들러.
    모든 기능은 부모 클래스인 BasePandasAgentHandler에 위임합니다.
    """
    def __init__(self):
        # 🎯 'cyber' 이라는 이름으로 부모 클래스를 초기화합니다.
        # 이 이름은 config.py에 정의된 설정 키와 일치해야 합니다.
        super().__init__(domain_name="cyber")
