# handlers/subject_satisfaction_handler.py
"""
벼리톡@경상남도인재개발원 - 교과목 만족도 핸들러 v6.0 (순수 pandas agent)
...
# 파일 경로
CSV_PATH = "data/satisfaction/subject_satisfaction.csv"
...
# 메시지
FALLBACK_MESSAGE = "현재 교과목 만족도 정보를 확인할 수 없습니다. 잠시 후 다시 시도하거나 경남인재개발원으로 직접 문의해주세요."
...
class SubjectSatisfactionHandler:
    ...
    def __init__(self):
        ...
        self._load_data()
        self._init_agent()
        logger.info("SubjectSatisfactionHandler v6.0 초기화 완료")
    ...
    def search_chunks(self, query: str) -> List[ChunkResult]:
        ...
        try:
            ...
            logger.error(f"교과목 만족도 분석 실패: {e}")
    ...
    def _load_data(self):
        ...
        for encoding in CSV_ENCODINGS:
            try:
                ...
                logger.info(f"교과목 만족도 CSV 로드 성공 ({encoding}): {len(self.df)}개 강의")
                return
    ...
    def _create_chunk_result(self, content: str) -> ChunkResult:
        ...
        return ChunkResult(
            chunk=TextChunk(
                content=content,
                metadata={
                    "source": "subject_satisfaction_analysis", 
                    "handler": "subject_satisfaction",
                    "data_source": "subject_satisfaction.csv"
                }
            ),
            confidence=0.5,  # 의미없는 고정값
            domain="subject_satisfaction"
        )
