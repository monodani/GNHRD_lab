...
"""
벼리톡@경상남도인재개발원 - 발행물 핸들러 v5.1 (직접 FAISS 접근)

직접 FAISS 접근으로 1700배 성능 향상
- 복잡한 담당부서 감지 로직 완전 제거
- 순수 거리→유사도 직접 계산
- 간소하고 우아한 코드 구조
"""
...
class PublishHandler:
    """발행물 검색 핸들러 (직접 FAISS 접근)"""
    
    def __init__(self):
        self.threshold = HANDLER_THRESHOLDS["publish"]
        self.department_info = DEPARTMENT_CONTACTS["publish"]
        self.index_manager = get_index_manager()
        logger.info(f"PublishHandler v5.1 초기화 완료 (임계값: {self.threshold})")
    
    def search_chunks(self, query: str) -> List[ChunkResult]:
        """직접 FAISS 접근으로 발행물 검색"""
        try:
