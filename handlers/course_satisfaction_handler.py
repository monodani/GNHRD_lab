# handlers/course_satisfaction_handler.py
"""
벼리톡@경상남도인재개발원 - 교육과정 만족도 핸들러 v6.0 (순수 pandas agent)
...
# 파일 경로
CSV_PATH = "data/satisfaction/course_satisfaction.csv"
...
# 메시지
FALLBACK_MESSAGE = "현재 교육과정 만족도 정보를 확인할 수 없습니다. 잠시 후 다시 시도하거나 경남인재개발원으로 직접 문의해주세요."
...
class CourseSatisfactionHandler:
    ...
    def __init__(self):
        ...
        self._load_data()
        self._init_agent()
        logger.info("CourseSatisfactionHandler v6.0 초기화 완료")
    ...
    def search_chunks(self, query: str) -> List[ChunkResult]:
        ...
        try:
            ...
            logger.error(f"교육과정 만족도 분석 실패: {e}")
    ...
    def _load_data(self):
        ...
        for encoding in CSV_ENCODINGS:
            try:
                ...
                logger.info(f"교육과정 만족도 CSV 로드 성공 ({encoding}): {len(self.df)}개 과정")
                return
    ...
    def _create_chunk_result(self, content: str) -> ChunkResult:
        ...
        return ChunkResult(
            chunk=TextChunk(
                content=content,
                metadata={
                    "source": "course_satisfaction_analysis", 
                    "handler": "course_satisfaction",
                    "data_source": "course_satisfaction.csv"
                }
            ),
            confidence=0.5,  # 의미없는 고정값
            domain="course_satisfaction"
        )
