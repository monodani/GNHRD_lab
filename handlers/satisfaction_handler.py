...
"""
벼리톡@경상남도인재개발원 - 만족도 조사 핸들러 v5.1 (직접 FAISS 접근)

직접 FAISS 접근으로 1700배 성능 향상
- 복잡한 만족도/순위 가중치 로직 완전 제거
- 순수 거리→유사도 직접 계산
- 간소하고 우아한 코드 구조
"""
...
class SatisfactionHandler:
    """만족도 조사 검색 핸들러 (직접 FAISS 접근)"""
    
    def __init__(self):
        self.threshold = HANDLER_THRESHOLDS["satisfaction"]
        self.department_info = DEPARTMENT_CONTACTS["satisfaction"]
        self.index_manager = get_index_manager()
        logger.info(f"SatisfactionHandler v5.1 초기화 완료 (임계값: {self.threshold})")
    
    def search_chunks(self, query: str) -> List[ChunkResult]:
        """직접 FAISS 접근으로 만족도 검색"""
        try:
