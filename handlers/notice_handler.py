...
"""
벼리톡@경상남도인재개발원 - 공지사항 핸들러 v5.1 (직접 FAISS 접근)

직접 FAISS 접근으로 1700배 성능 향상
- 복잡한 긴급도/마감일 가중치 로직 완전 제거
- 순수 거리→유사도 직접 계산
- 간소하고 우아한 코드 구조
"""
...
# 추가 설정 (필요시 확장)
# FUTURE: 공지사항 특화 로직이 필요하면 여기에 추가
...
class NoticeHandler:
    """공지사항 검색 핸들러 (직접 FAISS 접근)"""
    
    def __init__(self):
        self.threshold = HANDLER_THRESHOLDS["notice"]
        self.department_info = DEPARTMENT_CONTACTS["notice"]
        self.index_manager = get_index_manager()
        logger.info(f"NoticeHandler v5.1 초기화 완료 (임계값: {self.threshold})")
    
    def search_chunks(self, query: str) -> List[ChunkResult]:
        """직접 FAISS 접근으로 공지사항 검색"""
        try:
