...
"""
벼리톡@경상남도인재개발원 - 구내식당 메뉴 핸들러 v1.0

주간 식단표 및 구내식당 메뉴 검색
코랩 테스트 검증 완료 (5/5 성공, confidence: 0.406~0.535)
"""
...
class MenuHandler:
    ...
    def __init__(self):
        ...
        logger.info(f"MenuHandler 초기화 완료 (k={self.search_k})")
    ...
    def search_chunks(self, query: str) -> List[ChunkResult]:
        ...
        try:
            ...
            if not vectorstore:
                logger.warning("구내식당 메뉴 벡터스토어를 사용할 수 없습니다")
                return []
            ...
            logger.info(f"구내식당 메뉴 검색 완료: {len(chunk_results)}개 결과")
